...
2. Rename placeholder strings in file names and content such as `jaybro_dev_user_field` manually or with the helper script: 
    - `cd` into the directory and run `npm ci` (requires [Node.js](https://nodejs.org/)).
    - Run `npm run rename` and follow the prompts.
3. Copy the renamed `acf-jaybro-dev-user-field` directory into your theme or plugin.
4. Add this code to your theme or plugin to initialize the new field type, replacing 'acf-jaybro-dev-user-field' with the new folder name:

    ```php
    include_once __DIR__ . '/acf-jaybro-dev-user-field/init.php';    
    ```
...
- Change your field type's settings and behavior by editing `class-jaybro-acf-acf-field-jaybro-dev-user-field.php`.
...
If you remove `field.css` or `field.js`, be sure to also remove the `wp_enqueue` calls in `class-jaybro-acf-acf-field-jaybro-dev-user-field.php`.

If you plan to use the CSS and JS files, update the '1.0' version number in `class-jaybro-acf-acf-field-jaybro-dev-user-field.php` to use your theme or plugin version number or constant. This helps to invalidate browser caches for your field type assets when you update your theme or plugin.
